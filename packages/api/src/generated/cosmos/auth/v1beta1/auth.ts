...
export const ModuleAccount = {
	...
	decode(input: Reader | Uint8Array, length?: number): ModuleAccount {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					message.baseAccount = BaseAccount.decode(
						reader,
						reader.uint32()
					);
					break;
...
export const Params = {
	...
	fromJSON(object: any): Params {
		...
		if (
			...
			message.maxMemoCharacters = Long.fromString(
				object.maxMemoCharacters
			);
		} else {
		...
		if (
			...
			message.txSizeCostPerByte = Long.fromString(
				object.txSizeCostPerByte
			);
		} else {
