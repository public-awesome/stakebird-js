...
export const QueryContractHistoryResponse = {
	encode(
		...
		for (const v of message.entries) {
			ContractCodeHistoryEntry.encode(
				v!,
...
export const QueryCodeResponse = {
	...
	decode(input: Reader | Uint8Array, length?: number): QueryCodeResponse {
		...
		while (reader.pos < end) {
			...
			switch (tag >>> 3) {
				case 1:
					message.codeInfo = CodeInfoResponse.decode(
						reader,
						reader.uint32()
					);
					break;
...
export class QueryClientImpl implements Query {
	...
	Code(request: QueryCodeRequest): Promise<QueryCodeResponse> {
		...
		return promise.then((data) =>
			QueryCodeResponse.decode(new Reader(data))
		);
	}
	...
	Codes(request: QueryCodesRequest): Promise<QueryCodesResponse> {
		...
		return promise.then((data) =>
			QueryCodesResponse.decode(new Reader(data))
		);
	}
}
