...
const baseMining: object = {
  minter: "",
  lastTime: Long.ZERO,
};

const baseFaucetKey: object = {
  armor: "",
};
...
export const Mining = {
  encode(message: Mining, writer: Writer = Writer.create()): Writer {
    ...
    if (message.total !== undefined && message.total !== undefined) {
      Coin.encode(message.total, writer.uint32(26).fork()).ldelim();
    }
    return writer;
  },
  decode(input: Uint8Array | Reader, length?: number): Mining {
  ...
  toJSON(message: Mining): unknown {
    ...
    message.lastTime !== undefined && (obj.lastTime = (message.lastTime || Long.ZERO).toString());
...
export const FaucetKey = {
  ...
  decode(input: Uint8Array | Reader, length?: number): FaucetKey {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
