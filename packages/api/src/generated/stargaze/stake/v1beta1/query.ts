...
const baseQueryStakesRequest: object = {
  vendorId: 0,
  postId: "",
};
...
export class QueryClientImpl implements Query {
  ...
  Stakes(request: QueryStakesRequest): Promise<QueryStakesResponse> {
    ...
    const promise = this.rpc.request("stargaze.stake.v1beta1.Query", "Stakes", data);
    return promise.then(data => QueryStakesResponse.decode(new Reader(data)));
  }

  Stake(request: QueryStakeRequest): Promise<QueryStakeResponse> {
    const data = QueryStakeRequest.encode(request).finish();
    const promise = this.rpc.request("stargaze.stake.v1beta1.Query", "Stake", data);
    return promise.then(data => QueryStakeResponse.decode(new Reader(data)));
  }
...
export const QueryStakesRequest = {
  encode(message: QueryStakesRequest, writer: Writer = Writer.create()): Writer {
    writer.uint32(8).uint32(message.vendorId);
    writer.uint32(18).string(message.postId);
    return writer;
  },
  decode(input: Uint8Array | Reader, length?: number): QueryStakesRequest {
...
export const QueryStakeRequest = {
  encode(message: QueryStakeRequest, writer: Writer = Writer.create()): Writer {
    writer.uint32(8).uint32(message.vendorId);
    writer.uint32(18).string(message.postId);
    writer.uint32(26).string(message.delegator);
    return writer;
  },
  decode(input: Uint8Array | Reader, length?: number): QueryStakeRequest {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const QueryStakesResponse = {
  encode(message: QueryStakesResponse, writer: Writer = Writer.create()): Writer {
    for (const v of message.stakes) {
      Stake.encode(v!, writer.uint32(10).fork()).ldelim();
    }
    return writer;
  },
  decode(input: Uint8Array | Reader, length?: number): QueryStakesResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const QueryStakeResponse = {
  encode(message: QueryStakeResponse, writer: Writer = Writer.create()): Writer {
    if (message.stake !== undefined && message.stake !== undefined) {
      Stake.encode(message.stake, writer.uint32(10).fork()).ldelim();
    }
    return writer;
  },
  decode(input: Uint8Array | Reader, length?: number): QueryStakeResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
